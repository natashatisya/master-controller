...
try {
    [System.Environment]::SetEnvironmentVariable("MINIO_ROOT_USER", $MinioRootUser, "User")
    [System.Environment]::SetEnvironmentVariable("MINIO_ROOT_PASSWORD", $MinioRootPassword, "User")
    $env:MINIO_ROOT_USER = $MinioRootUser
    $env:MINIO_ROOT_PASSWORD = $MinioRootPassword
    Write-Host "  Environment variables set!" -ForegroundColor Green
} catch {
    Write-Host "  Warning: Could not set env vars, using session vars only: $_" -ForegroundColor Yellow
}
